...
udp_port=$(expr $BASE_CL_PORT)
tcp_port=$(expr $BASE_CL_PORT + 1)
http_port=$(expr $BASE_CL_HTTP_PORT)
log_file=${DATA_DIR_MOUNT_PATH}/beacon_node.log

echo "Started the lighthouse beacon node $node_ip which is now listening at ports $udp_port and $tcp_port, and http at port $http_port. You can see the log at $log_file"
...
ssh ${node_ip} docker run -d \
    ...
    --execution-endpoint http://${node_ip}:$(expr $BASE_EL_RPC_PORT) \
...
if test $? -ne 0; then
    node_error "The lighthouse beacon node $node_ip returns an error. The last 10 lines of the log file is shown below.\n\n$(ssh ${node_ip} docker logs -n 10 $container_name)"
    exit 1
fi
